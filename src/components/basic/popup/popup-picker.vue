<template>
  <div class="vux-cell-box">
    <div v-show="showCell" v-tap="onClick" class="weui-cell vux-tap-active" :class="{'weui-cell_access': !disabled}">
      <div class="weui-cell__hd">
        <slot name="title" label-class="weui-label" :label-style="labelStyles" :label-title="title">
          <label v-if="title" class="weui-label" :class="labelClass" :style="labelStyles" v-html="title"></label>
        </slot>
      ...
      <div class="vux-cell-primary vux-popup-picker-select-box">
        <div class="vux-popup-picker-select" :style="{textAlign: valueTextAlign}">
          <span v-if="!displayFormat && !showName && value.length" class="vux-popup-picker-value vux-cell-value">{{value | array2string}}</span>
          <span v-if="!displayFormat && showName && value.length" class="vux-popup-picker-value vux-cell-value">{{value | value2name(data)}}</span>
          <span v-if="displayFormat && value.length" class="vux-popup-picker-value vux-cell-value">{{ displayFormat(value, value2name(value, data)) }}</span>
          <span v-if="!value.length && placeholder" class="vux-popup-picker-placeholder vux-cell-placeholder" v-text="placeholder"></span>
        </div>
      </div>
    ...
    <div v-transfer-dom="isTransferDom">
      <popup :id="`vux-popup-picker-${uuid}`" v-model="showValue" class="vux-popup-picker" :popup-style="popupStyle" @on-hide="onPopupHide" @on-show="onPopupShow">
        <div class="vux-popup-picker-container">
          <popup-header :left-text="cancelText || $t('cancel_text')" :right-text="confirmText || $t('confirm_text')" :title="popupTitle" @on-click-left="onHide(false)" @on-click-right="onHide(true)"></popup-header>
          <picker v-model="tempValue" :data="data" :columns="columns" :fixed-columns="fixedColumns" :container="'#vux-popup-picker-'+uuid" :column-width="columnWidth" @on-change="onPickerChange"></picker>
        </div>
      </popup>
    </div>
  ...
  computed: {
    labelStyles() {
      return {
        display: 'block',
        width:
          (this.$parent && (this.$parent.labelWidth || this.$parent.$parent.labelWidth)) || 'auto',
        textAlign: this.$parent && (this.$parent.labelAlign || this.$parent.$parent.labelAlign),
        marginRight:
          this.$parent && (this.$parent.labelMarginRight || this.$parent.$parent.labelMarginRight)
      }
    },
    labelClass() {
      return {
