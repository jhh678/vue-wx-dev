import passiveSupported from '@/utils/tools/passive-supported'
...
const popupDialog = function(option) {
  ...
  if (!!document.querySelectorAll('.ui-popup-mask').length <= 0) {
    this.divMask = document.createElement('a')
    this.divMask.className = 'ui-popup-mask'
  ...
  div.className += ` ui-popup-dialog ui-popup-dialog-${this.uuid}`
  if (!this.params.hideOnBlur) {
    div.className += ' ui-popup-mask-disabled'
  }
  ...
  this.container = document.querySelector('.ui-popup-dialog-' + this.uuid)
  this.mask = document.querySelector('.ui-popup-mask')
...
popupDialog.prototype.show = function() {
  if (this.params.showMask) {
    this.mask.classList.add('ui-popup-show')
    this.mask.style['zIndex'] = 500
  }
  this.container.classList.add('ui-popup-show')
...
popupDialog.prototype.hide = function(shouldCallback = true) {
  this.container.classList.remove('ui-popup-show')
  if (!document.querySelector('.ui-popup-dialog.ui-popup-show')) {
    this.mask.classList.remove('ui-popup-show')
    setTimeout(() => {
